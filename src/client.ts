...
export class Client {
  ...
  private server$ = new Observable<WebSocket>((observer) => {
    const subdomains = this.options.tunnels.map(
      (v) => `${v.subdomain}|${v.label}`,
    );
    ...
    retry({
      delay: (err) => {
        const delaySeconds = Math.round(
          Math.min(600, Math.pow(1.8, this.retryCount - 1)),
        );
        this.options.logger?.error(
          `[nora-link] connection error ${this.retryCount}: ${err}`,
        );
        this.options.logger?.info(
          `[nora-link] retrying in ${delaySeconds} sec`,
        );
        this.retryCount++;
  ...
  readonly idle$ = new BehaviorSubject<null | {
    db: string;
    dbKey: string;
    apiKey: string;
    token: string;
  ...
  private httpRequest({
    ...
    return new Observable<never>((reqObserver) => {
      this.options.logger?.trace(`[nora-link] START ${type} ${method} ${url}`);

      const tunnel = this.options.tunnels.find(
        (v) => v.subdomain === subdomain,
      );
      if (!tunnel) {
      ...
      if (type === "ws") {
        ...
        const handleUpgrade$ = new Observable<{
          res: IncomingMessage;
          socket: Socket;
        }>((observer) => {
      ...
      if (type === "http") {
        const handleResponse$ = new Observable<IncomingMessage>((observer) => {
          ...
          concatMap(
            (res) =>
              new Observable<Observable<void>>((observer) => {
                const head$ = send(
                  MessageTypes.HEAD,
                  Buffer.from(
                    JSON.stringify({
        ...
        return merge(handleResponse$, rx$).subscribe(reqObserver);
      }
      ...
      finalize(() => {
        this.options.logger?.trace(`[nora-link] DONE ${type} ${method} ${url}`);
      }),
    );
  }
}
