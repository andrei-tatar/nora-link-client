...
program
  ...
  .option<LogLevel>(
    "-l, --log <log-level>",
    `min log level to display. Options: ${LOG_LEVEL.join(",")}`,
    (value, previous) => {
  ...
  .requiredOption(
    "-f, --forward <subdomain|label|url...>",
    "Tunnel subdomain to local url. Label is optional",
  )
  .requiredOption("-k, --key <api-key>", "Api Key")
  .addHelpText(
    "afterAll",
    'To tunnel a local port to the subdomain `test`, you can use `-f "test|localhost:3000"`',
  )
  .addHelpText(
    "afterAll",
    'To tunnel an ip to the subdomain `test`, you can use `-f "test|192.168.1.130"`',
  )
  .addHelpText(
    "afterAll",
